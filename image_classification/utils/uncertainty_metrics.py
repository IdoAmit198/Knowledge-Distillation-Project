...
def gini(samples_certainties):
    ...
    if torch.min(array) < 0:
        # Values cannot be negative:
        array -= torch.min(array)
    # Values cannot be 0:
    array += 0.0000001
...
def ECE_calc(samples_certainties, num_bins=15, bin_boundaries_scheme=None):
    ...
    for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
        ### changed, validate this change
        # bin_indices = torch.logical_and(samples_certainties[0] <= bin_upper, samples_certainties[0] > bin_lower)
        # print(f"samples_certainties[0] device is {(samples_certainties[0].device)}")
        # print(f"bin_upper device is {(bin_upper.device)}")
        # print(f"bin_lower device is {(bin_lower.device)}")
        cur_device = samples_certainties.device
        upper = samples_certainties[0] <= bin_upper
        lower = samples_certainties[0] > bin_lower
        bin_indices = upper & lower
